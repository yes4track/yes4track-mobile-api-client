...
class BsonValue {
  ...
  @override
  String toString() {
    return 'BsonValue[asBoolean=$asBoolean, asBsonArray=$asBsonArray, asBsonBinaryData=$asBsonBinaryData, asBsonDateTime=$asBsonDateTime, asBsonDocument=$asBsonDocument, asBsonJavaScript=$asBsonJavaScript, asBsonJavaScriptWithScope=$asBsonJavaScriptWithScope, asBsonMaxKey=$asBsonMaxKey, asBsonMinKey=$asBsonMinKey, asBsonNull=$asBsonNull, asBsonRegularExpression=$asBsonRegularExpression, asBsonSymbol=$asBsonSymbol, asBsonTimestamp=$asBsonTimestamp, asBsonUndefined=$asBsonUndefined, asBsonValue=$asBsonValue, asByteArray=$asByteArray, asDateTime=$asDateTime, asDecimal=$asDecimal, asDecimal128=$asDecimal128, asDouble=$asDouble, asGuid=$asGuid, asInt32=$asInt32, asLocalTime=$asLocalTime, asInt64=$asInt64, asNullableBoolean=$asNullableBoolean, asNullableDateTime=$asNullableDateTime, asNullableDecimal=$asNullableDecimal, asNullableDecimal128=$asNullableDecimal128, asNullableDouble=$asNullableDouble, asNullableGuid=$asNullableGuid, asNullableInt32=$asNullableInt32, asNullableInt64=$asNullableInt64, asNullableObjectId=$asNullableObjectId, asObjectId=$asObjectId, asRegex=$asRegex, asString=$asString, asUniversalTime=$asUniversalTime, bsonType=$bsonType, isBoolean=$isBoolean, isBsonArray=$isBsonArray, isBsonBinaryData=$isBsonBinaryData, isBsonDateTime=$isBsonDateTime, isBsonDocument=$isBsonDocument, isBsonJavaScript=$isBsonJavaScript, isBsonJavaScriptWithScope=$isBsonJavaScriptWithScope, isBsonMaxKey=$isBsonMaxKey, isBsonMinKey=$isBsonMinKey, isBsonNull=$isBsonNull, isBsonRegularExpression=$isBsonRegularExpression, isBsonSymbol=$isBsonSymbol, isBsonTimestamp=$isBsonTimestamp, isBsonUndefined=$isBsonUndefined, isDateTime=$isDateTime, isDecimal128=$isDecimal128, isDouble=$isDouble, isGuid=$isGuid, isInt32=$isInt32, isInt64=$isInt64, isNumeric=$isNumeric, isObjectId=$isObjectId, isString=$isString, isValidDateTime=$isValidDateTime, rawValue=$rawValue, ]';
  }

  BsonValue.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
  ...
  static List<BsonValue> listFromJson(List<dynamic> json) {
    return json == null
        ? new List<BsonValue>()
        : json.map((value) => new BsonValue.fromJson(value)).toList();
  }

  static Map<String, BsonValue> mapFromJson(
      Map<String, Map<String, dynamic>> json) {
    var map = new Map<String, BsonValue>();
    if (json != null && json.length > 0) {
      json.forEach((String key, Map<String, dynamic> value) =>
          map[key] = new BsonValue.fromJson(value));
    }
    return map;
  }
}
